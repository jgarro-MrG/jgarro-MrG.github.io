...
describe('Twiddler', () => {
  ...
  it("should show the user's timeline when the username is clicked", async () => {
    ...
    // Ensure we are in the 'all tweets' view first for a clean state
    if ($('#reset-button').is(':visible')) {
      $('#reset-button').trigger('click');
      await wait(200); // Brief wait for reset to render, adjust if necessary
    }

    const $tweets = $('.tweet');
    if ($tweets.length === 0) {
      assert.fail('[TEST] No tweets found on the page to start the test.');
    }
    ...
    if (!$username.length) {
      assert.fail('[TEST] Expected to find an element with the class "username" inside the random tweet.');
    }

    const clickedUsernameData = $username.data('username'); // e.g., "sharksforcheap"
    const clickedUsernameText = $username.text();          // e.g., "@sharksforcheap"
    ...
    $tweetsAfterClick.each((_index, tweet) => {
      const tweetUsernameDataAttr = $(tweet).find('.username').data('username');
      const tweetUsernameTextContent = $(tweet).find('.username').text();
      expect(tweetUsernameDataAttr, `Tweet by ${tweetUsernameTextContent} should be by user ${clickedUsernameData}`).to.equal(clickedUsernameData);
    });
  });
